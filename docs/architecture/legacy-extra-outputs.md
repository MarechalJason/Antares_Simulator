...
The `price` formula negates the stored dual: with the legacy balance-constraint sign convention, `CoutsMarginauxDesContraintes` holds the **negative** of the marginal price (the legacy outputs print `-CoutsMarginauxHoraires`, and the adequacy-patch post-processing stores `-unsuppliedEnergyCost` to mean "price = unsupplied energy cost"). Note that dual-based outputs are anchored on a recorded *constraint*, but the dual is still read by index — a constraint-side lookup view does not exist yet because no output so far needs to find a dual from a variable anchor.
...
- **Remaining dual outputs.** The constraint-recording infrastructure exists, but only `AreaBalance` is recorded. Link `capacity_shadow_price`/`congestion_fee`, `hydro_shadow_price` and the profit outputs need their constraints (`FlowDissociation`, `FinalStockExpression`, ...) recorded the same way — and the profit outputs additionally need a constraint-side lookup view, since they are anchored on variables and must *find* the matching dual.
