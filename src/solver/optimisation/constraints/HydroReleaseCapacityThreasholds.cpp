...
void HydroReleaseCapacityThreasholds::add(int pays, int cluster, int pdt)
{
    ...
    if (!data.Simulation)
    {
        ...
        // 15 (c) (1) : H - Sum(H^on_re-) >= Hmin
        {
            ...
            if (builder.NumberOfVariables() > 0)
            {
                ...
                data.CorrespondanceCntNativesCntOptim[pdt]
                  .reservesIndices()
                  .HydroReleaseCapacityThreasholdsMin[globalClusterIdx]
                  = builder.data.nombreDeContraintes;
                ConstraintNamer namer(builder.data.NomDesContraintes);
                const int hourInTheYear = builder.data.weekInTheYear * 168 + pdt;
                namer.UpdateTimeStep(hourInTheYear);
                namer.UpdateArea(builder.data.NomsDesPays[pays]);
                namer.HydroReleaseCapacityThreasholdsDown(builder.data.nombreDeContraintes,
                                                          "Hydro");
                builder.build();
            }
        }
        ...
        // 15 (c) (2) :  H + Sum(H^on_re+) <= Hmax
        {
            ...
            if (builder.NumberOfVariables() > 0)
            {
                ...
                data.CorrespondanceCntNativesCntOptim[pdt]
                  .reservesIndices()
                  .HydroReleaseCapacityThreasholdsMax[globalClusterIdx]
                  = builder.data.nombreDeContraintes;
                ConstraintNamer namer(builder.data.NomDesContraintes);
                const int hourInTheYear = builder.data.weekInTheYear * 168 + pdt;
                namer.UpdateTimeStep(hourInTheYear);
                namer.UpdateArea(builder.data.NomsDesPays[pays]);
                namer.HydroReleaseCapacityThreasholdsUp(builder.data.nombreDeContraintes, "Hydro");
                builder.build();
            }
        }
    }
