      ...
      unsigned int numSpace)
    {
        for (unsigned int i = 0; i <= state.study.runtime.rangeLimits.hour[Data::rangeEnd]; ++i)
        {
            ...
            state.thermalClusterPMinOfTheClusterForYear[i]
              += pMinBySpace[numSpace][(state.thermalCluster->enabledIndex * HOURS_PER_YEAR) + i];
        }
    }
};

using VCardProductionByDispatchablePlant =
  VCardDispatchablePlantByClusterBase<ProductionByDispatchablePlantTraits>;
